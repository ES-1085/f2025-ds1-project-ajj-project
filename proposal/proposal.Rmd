...
```{r dimensions}
dim(GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD)
dim(GDP_Per_Capita_2018_2024_Annual_OECD_Non_OECD)
```

```{r glimpse}
glimpse(GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD)
...
names(GDP_Growth_Clean_Concise_Dataset_2018_2024_Annual)[1] <- "Country"
names(GDP_Per_Capita_2018_2024_Annual_OECD_Non_OECD)[1] <- "Country"
```

```{r convert-to-long-format-for-time-series-analysis}
# make all column names characters  
names(GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD) <-
  as.character(names(GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD))

# convert all "year" columns (2018–2024) to characters before pivoting
GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD <-
  GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD %>%
  mutate(across(matches("^20"), as.character))

# pivot longer using the original "Time Period" column  
gdp_growth_long <- GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD %>%
  pivot_longer(
...
```{r identify-countries-post-pandemic (2021-24)}
...
plot1_gdp_trends <- gdp_growth_long %>%
  filter(`Time period` %in% focus_countries) %>%
  ggplot(aes(x = Year, y = GDP_Growth, color = `Time period`, group = `Time period`)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "GDP Growth Trends (2018–2024)",
    subtitle = "Tracking Pre-, During-, and Post-COVID Economic Recovery",
    x = "Year",
    y = "GDP Growth (%)",
    color = "Country",
...
plot2_post_pandemic <- post_pandemic_growth %>%
  ...
  ggplot(aes(x = reorder(`Time period`, avg_growth_post_pandemic), y = avg_growth_post_pandemic)) +
  geom_col(fill = "#0072B2") +
  coord_flip() +
  labs(
    title = "Top 15 Countries by Average GDP Growth (2021–2024)",
