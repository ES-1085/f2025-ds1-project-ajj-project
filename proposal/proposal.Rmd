...
```{r load-data}
Annual_GDP_Growth_OECD_and_non_OECD <- read_excel("/cloud/project/data/Annual_GDP_Growth_OECD_and_non_OECD.xlsx")
# View(Annual_GDP_Growth_OECD_and_non_OECD)

Annual_GDP_Per_Capita_OECD_and_Non_OECD_xlsx <- read_excel("/cloud/project/data/Annual GDP Per Capita OECD and Non-OECD.xlsx.xlsx")
# View(Annual_GDP_Per_Capita_OECD_and_Non_OECD_xlsx)
```

```{r dimensions}
dim(Annual_GDP_Growth_OECD_and_non_OECD)
dim(Annual_GDP_Per_Capita_OECD_and_Non_OECD)
```

```{r glimpse}
glimpse(Annual_GDP_Growth_OECD_and_non_OECD)
glimpse(Annual_GDP_Per_Capita_OECD_and_Non_OECD)
```
...
names(Annual_GDP_Per_Capita_OECD_and_Non_OECD)[1] <- "Time Period"
```

```{r convert-to-long-format-for-time-series-analysis, message=FALSE}
# make all column names characters  
names(Annual_GDP_Growth_OECD_and_non_OECD) <-
  as.character(names(Annual_GDP_Growth_OECD_and_non_OECD))

# convert all "year" columns (2018–2024) to characters before pivoting
Annual_GDP_Growth_OECD_and_non_OECD <-
  Annual_GDP_Growth_OECD_and_non_OECD %>%
  mutate(across(matches("^20"), as.character))

# pivot longer using the original "Time Period" column  
gdp_growth_long <- Annual_GDP_Growth_OECD_and_non_OECD %>%
  pivot_longer(
