...
# Fix column names
names(GDP_Per_Capita) <- as.character(names(GDP_Per_Capita))

# Clean GDP per capita data
GDP_Per_Capita_clean <- GDP_Per_Capita %>%
  select(-`...2`) %>%                     # remove useless column
  rename(Country = `Time period`) %>%     # rename to match gdp_growth_long
  mutate(across(matches("^20"), as.character)) %>% 
  pivot_longer(
    cols = matches("^20"),
    names_to = "Year",
...
merged_data <- gdp_growth_long %>%
  left_join(GDP_Per_Capita_clean, by = c("Country", "Year")) %>%
  filter(
    !is.na(GDP_Growth),
...
anti_join(gdp_growth_long, GDP_Per_Capita_clean, by = c("Country", "Year"))


plot4_animation <- merged_data %>%
  ggplot(aes(
    x = GDP_Per_Capita,
    y = GDP_Growth,
    color = Country,
