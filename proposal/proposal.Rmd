...
names(GDP_Growth_Filtered_Concise_Dataset_2018_2024_Annual_OECD_Non_OECD)[1] <- "Country"
...
```{r visualization}
# line plot: GDP growth over time
ggplot(gdp_growth_long, aes(x = Year, y = GDP_Growth, group = Country)) +
  geom_line(alpha = 0.3) +
  geom_smooth(aes(group = 1), color = "blue", se = FALSE, linewidth = 1.2) +
  labs(title = "Global GDP Growth Trends (2018–2024)",
       x = "Year", y = "GDP Growth (%)") +
  theme_minimal()

# box plot: distribution by year
ggplot(gdp_growth_long, aes(x = as.factor(Year), y = GDP_Growth)) +
  geom_boxplot(fill = "lightblue", outlier.color = "red") +
  labs(title = "Distribution of GDP Growth by Year",
       x = "Year", y = "GDP Growth (%)") +
  theme_minimal()

# bar chart: average post-pandemic GDP growth
ggplot(post_pandemic_growth[1:15, ], aes(x = reorder(Country, avg_growth_post_pandemic),
                                         y = avg_growth_post_pandemic)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Top 15 Countries with Strongest Post-Pandemic GDP Growth",
       x = "Country", y = "Average GDP Growth (2021–2024 %)") +
  theme_minimal()
```
