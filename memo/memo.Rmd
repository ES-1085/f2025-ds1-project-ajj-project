...
library(tidyverse)
library(broom)
library(gganimate) # for optional animation
library(scales)
```
...
```{r}
gdp <- read_csv("data/GDP_Growth_Clean_Concise_Dataset_2018_2024_Annual.csv")
glimpse(gdp)
```
...
Description:
Categorizing helps color/facet plots meaningfully. The thresholds (4% and 0%) are chosen based on typical GDP growth ranges—feel free to adjust.
